...
| Shareable URL state accuracy | The URL could lag behind current workspace edits, so copied links could restore stale command/format state. | Added live share-state syncing from the current workspace so copied URLs restore the latest input, command, and formats. | `components/MillWorkspace` |
